...
from .utils import Filenamer, Enforcer, concatenateCatalogs, checkIdLists, \
    addPatchColumn, calibrateCoaddSourceCatalog, writeParquet, getRepoInfo
...
class ColorAnalysisTask(CmdLineTask):
    ...
    def run(self, patchRefsByFilter):
        ...
        self.transformCatalogs(unforcedCatalogsByFilter, self.config.transforms,
    ...
    def plotStarColorColor(self, catalogs, filenamer, dataId, butler=None, camera=None, tractInfo=None,
        ...
        def color(c1, c2, good=good):
            return (mags[c1] - mags[c2])[good]
        ...
        if filters.issuperset(set(("HSC-G", "HSC-R", "HSC-I"))):
            ...
            colorColorPolyFitPlot(dataId, filenamer(dataId, description="gri-wFit", style="fit"),
            ...
            colorColorPolyFitPlot(dataId, filenamer(dataId, description="gri-xFit", style="fit"),
                                  self.log, color("HSC-G", "HSC-R"), color("HSC-R", "HSC-I"),
                                  "g - r", "r - i", xRange=(-0.5, 2.0), yRange=(-0.5, 2.0),
                                  order=1, xFitRange=(xFitRange1, xFitRange2), camera=camera,
                                  hscRun=hscRun, unitScale=self.unitScale)
            ...
            poly = colorColorPolyFitPlot(dataId, filenamer(dataId, description="gri", style="fit"), self.log,
                                         ...
                                         xRange=(-0.5, 2.0), yRange=(-0.5, 2.0), order=3,
                                         xFitRange=(0.3, 1.1), camera=camera, hscRun=hscRun,
        ...
        if filters.issuperset(set(("HSC-R", "HSC-I", "HSC-Z"))):
            ...
            colorColorPolyFitPlot(dataId, filenamer(dataId, description="riz-yFit", style="fit"),
...
def colorColorPolyFitPlot(dataId, filename, log, xx, yy, xLabel, yLabel, xRange=None, yRange=None, order=1,
    ...
    def calculateDistance2(x1, y1, x2, poly=poly):
...
class ColorColorDistance(object):
    ...
    def __call__(self, catalog):
        ...
        for i, (x, y) in enumerate(zip(xx, yy)):
            ...
            distance2[i] = min(self.calculateDistance2(x, y, np.real(rr)) for
                               rr in roots if np.real(rr) == rr)
        return np.sqrt(distance2)*np.where(yy >= self.poly(xx), 1.0, -1.0)

    def calculateDistance2(self, x1, y1, x2):
        return (x2 - x1)**2 + (self.poly(x2) - y1)**2
