...
class CoaddAnalysisTask(CmdLineTask):
    ...
    def runDataRef(self, patchRefList, subdir="", cosmos=None, butlerGen2=None):
        ...
        # found).
        dataId = patchRefList[0]["dataId"] if repoInfo.isGen3 else patchRefList[0].dataId
        if not repoInfo.isGen3:
            if patchRefList[0].datasetExists(self.config.coaddName + dataset):
                haveForced = True
        else:
            try:
                patchRefList[0]["butler"].getURI(self.config.coaddName + dataset, dataId=dataId)
                haveForced = True
            except LookupError:
                haveForced = False
        ...
        if not repoInfo.isGen3:
            ...
            patchList = [patchRef["dataId"]["patch"] for patchRef in patchRefList]
            patchIdList = [patchRef["patchId"] for patchRef in patchRefList]
