...
def createDataList(analysisStr, logFile, tracts, band):
    ...
    quantityList = []
    shortNameColorsList = []
    # This is for the Distance to polynomial entry that comes with colorPerp but is associated with
    # the same shortName (which is just the color, e.g. gri)
    ...
    with open(logFile, "r") as inFile:
        ...
        for line in inFile:
            ...
            if line.startswith(statsStartStr):
                ...
                if dataIdDict.has_key("patch"):
                    patch = dataIdDict["patch"]
                if dataIdDict.has_key("field"):
                    field = dataIdDict["field"]
                if dataIdDict.has_key("visit"):
                ...
                entry = VisitEntry(field, tract, patch, visit, band, quantityName, starStatsDict, galStatsDict)
...
def printStatsSum(analysisStr, lsstLogFile, hscLogFile, tracts, band, outLogFile, outShortLogFile):
    ...
    if lsstVisitEntryList[0].patch is not None:
        for index, (lsstVisitEntry, hscVisitEntry) in enumerate(zip(lsstVisitEntryList, hscVisitEntryList)):
            # HSC forced catalogs do not include Gaussian fluxes, so need to insert Null entry
    ...
    if len(lsstVisitEntryList) != len(hscVisitEntryList):
        ...
        # Insert Nan entries where HSC has missing ones (something is going wrong with reading in some of
        # the match files...or perhaps they just do not exist for certain visit/ccd
        index = 0
        looping = True
        while looping:
            for index, (lsstVisitEntry, hscVisitEntry) in enumerate(zip(lsstVisitEntryList, hscVisitEntryList)):
    ...
    with open(outLogFile, "w") as outFile, open(outShortLogFile, "w") as outShortFile:
        for lsstQuantityName in lsstQuantityList:
            ...
            outShortFile.write("#                filter   mean    [HSC]     stdev   [HSC]    median   [HSC]      num     [HSC]     numTot    [HSC]  NumEntries" + "\n")
            outFile.write("# Stars: " + lsstQuantityName + "\n")
            outFile.write("# tract  visit   filter   mean    [HSC]     stdev   [HSC]    median   [HSC]      num     [HSC]     numTot    [HSC]" + "\n")
            ...
            for lsstVisitEntry, hscVisitEntry in zip(lsstVisitEntryList, hscVisitEntryList):
                if lsstVisitEntry.quantityName != hscVisitEntry.quantityName:
                    raise RuntimeError("Quantity names for visit {0:d} do not match! {1:s} vs {2:s}".format(
                            lsstVisitEntry.visit, lsstVisitEntry.quantityName, band, hscVisitEntry.quantityName))
                # outFile.write("# {:s}".format(lsstVisitEntry.field)
                formatStr = "{0:6d} {1:7d} {2:>7s} {3:>7.3F} [{4:7.3F}] {5:>7.3F} [{6:7.3F}] {7:>7.3F} [{8:7.3F}] {9:>8d} [{10:8d}] {11:>8d} [{12:8d}]\n"
                if lsstVisitEntry.quantityName == lsstQuantityName:
                    ...
                    if lsstVisitEntry.starStatsDict["num"] > 0:
                        ...
                        lsstWgtMeanMean += lsstVisitEntry.starStatsDict["mean"]*lsstVisitEntry.starStatsDict["num"]
                        lsstWgtMeanStdev += lsstVisitEntry.starStatsDict["stdev"]*lsstVisitEntry.starStatsDict["num"]
                        lsstWgtMeanMedian += lsstVisitEntry.starStatsDict["median"]*lsstVisitEntry.starStatsDict["num"]
                    ...
                    if hscVisitEntry.starStatsDict["num"] > 0:
                        ...
                        hscWgtMeanMean += hscVisitEntry.starStatsDict["mean"]*hscVisitEntry.starStatsDict["num"]
                        hscWgtMeanStdev += hscVisitEntry.starStatsDict["stdev"]*hscVisitEntry.starStatsDict["num"]
                        hscWgtMeanMedian += hscVisitEntry.starStatsDict["median"]*hscVisitEntry.starStatsDict["num"]
            ...
            if lsstNumSum > 0:
                ...
                lsstWgtMeanMean, lsstWgtMeanStdev, lsstWgtMeanMedian = float("nan"), float("nan"), float("nan")
            if hscNumSum > 0:
            ...
            if lsstNumVisits > 1:
                ...
                outFile.write(("{0:14s} {1:>7s} {2:>7.3F} [{3:7.3F}] {4:>7.3F} [{5:7.3F}] {6:>7.3F} [{7:7.3F}] {8:>8d} [{9:8d}] {10:>8d} [{11:8d}]\n".format("# straight avg", bandStrDict[band], lsstMeanMean, hscMeanMean, lsstMeanStdev, hscMeanStdev, lsstMeanMedian, hscMeanMedian, lsstNumSum, hscNumSum, lsstTotalSum, hscTotalSum)))
                outFile.write(("{0:14s} {1:>7s} {2:>7.3F} [{3:7.3F}] {4:>7.3F} [{5:7.3F}] {6:>7.3F} [{7:7.3F}] {8:>8d} [{9:8d}] {10:>8d} [{11:8d}]\n".format("# weighted avg", bandStrDict[band], lsstWgtMeanMean, hscWgtMeanMean, lsstWgtMeanStdev, hscWgtMeanStdev, lsstWgtMeanMedian, hscWgtMeanMedian, lsstNumSum, hscNumSum, lsstTotalSum, hscTotalSum)))
            outFile.write(lineSplitStr)

            outShortFile.write(("{0:14s} {1:>7s} {2:>7.3F} [{3:7.3F}] {4:>7.3F} [{5:7.3F}] {6:>7.3F} [{7:7.3F}] {8:>8d} [{9:8d}] {10:>8d} [{11:8d}] {12:>8d}\n".format("# straight avg", bandStrDict[band], lsstMeanMean, hscMeanMean, lsstMeanStdev, hscMeanStdev, lsstMeanMedian, hscMeanMedian, lsstNumSum, hscNumSum, lsstTotalSum, hscTotalSum, lsstNumVisits)))
            if lsstNumVisits > 1:
                outShortFile.write(("{0:14s} {1:>7s} {2:>7.3F} [{3:7.3F}] {4:>7.3F} [{5:7.3F}] {6:>7.3F} [{7:7.3F}] {8:>8d} [{9:8d}] {10:>8d} [{11:8d}] {12:>8d}\n".format("# weighted avg", bandStrDict[band], lsstWgtMeanMean, hscWgtMeanMean, lsstWgtMeanStdev, hscWgtMeanStdev, lsstWgtMeanMedian, hscWgtMeanMedian, lsstNumSum, hscNumSum, lsstTotalSum, hscTotalSum, lsstNumVisits)))
...
def printCompareStatsSum(analysisStr, lsstLogFile, tracts, band, outLogFile, outShortLogFile):
    ...
    with open(outLogFile, "w") as outFile, open(outShortLogFile, "w") as outShortFile:
        for lsstQuantityName in lsstQuantityList:
            outShortFile.write("# Stars: " + lsstQuantityName + "\n")
            outShortFile.write("#               filter   mean    stdev   median     num    numTot  NumEntries" + "\n")
            outFile.write("# Stars: " + lsstQuantityName + "\n")
            ...
            if lsstNumVisits > 1:
                outFile.write(lineSplitStr)
                outFile.write(("{0:14s} {1:>7s} {2:>7.3F} {3:>7.3F} {4:>7.3F} {5:>8d} {6:>8d}\n".format("# straight avg", bandStrDict[band], lsstMeanMean, lsstMeanStdev, lsstMeanMedian, lsstNumSum, lsstTotalSum)))
                outFile.write(("{0:14s} {1:>7s} {2:>7.3F} {3:>7.3F} {4:>7.3F} {5:>8d} {6:>8d}\n".format("# weighted avg", bandStrDict[band], lsstWgtMeanMean, lsstWgtMeanStdev, lsstWgtMeanMedian, lsstNumSum, lsstTotalSum)))
            outFile.write(lineSplitStr)

            outShortFile.write(("{0:14s} {1:>7s} {2:>7.3F} {3:>7.3F} {4:>7.3F} {5:>8d} {6:>8d} {7:>8d}\n".format("# straight avg", bandStrDict[band], lsstMeanMean, lsstMeanStdev, lsstMeanMedian, lsstNumSum, lsstTotalSum, lsstNumVisits)))
            if lsstNumVisits > 1:
                outShortFile.write(("{0:14s} {1:>7s} {2:>7.3F} {3:>7.3F} {4:>7.3F} {5:>8d} {6:>8d} {7:>8d}\n".format("# weighted avg", bandStrDict[band], lsstWgtMeanMean, lsstWgtMeanStdev, lsstWgtMeanMedian, lsstNumSum, lsstTotalSum, lsstNumVisits)))
            outShortFile.write(lineSplitStr)
...
analysisStrs = ["visitAnalysis", "compareVisitAnalysis", "coaddAnalysis", "compareCoaddAnalysis", "colorAnalysis"]
lineSplitStr = "#===================================================================================================================" + "\n"
